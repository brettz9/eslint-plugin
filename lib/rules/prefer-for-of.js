...
function isAssignee(startNode) {
  ...
  while (node && node.parent && !SENTINEL_TYPE.test(node.type)) {
    ...
    const assignee =
            ...
            // istanbul ignore next -- No `AssignmentPattern` as child of `MemberExpression`?
            (parent.type === 'AssignmentPattern' && parent.left === node) ||
            // istanbul ignore next -- No `VariableDeclarator` as child of `MemberExpression`?
...
function getVariableByName(context, name) {
  ...
  // istanbul ignore next -- Apparently unreachable
  return null
}
