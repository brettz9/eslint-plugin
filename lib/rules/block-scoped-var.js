 ...
 * @param {import('eslint').Scope.Definition} def - A definition to check.
 * @param {import('eslint').Scope.Definition[]} defs - A definition list which includes `def`.
 * @param {import('eslint').Scope.Variable} variable - A variable which is defined by the definition.
...
class PseudoScope {
  /**
     * @param {import('eslint').Scope.Definition} def - A declaration.
     */
  constructor(def) {
    let node = null
  ...
  /**
     ...
     * @param {import('eslint').Scope.Variable} variable - A variable to create.
  ...
  /**
     ...
     * @param {import('eslint').Scope.Reference} reference - A reference to find.
     * @returns {PseudoScope|null} A containing scope of the reference.
     */
  static findScope(scopes, reference) {
    const range = /** @type {[number, number]} */ (reference.identifier.range)
...
export default {
  meta: {
    docs: {
      ...
      recommended: false,
      url:
                'https://github.com/brettz9/eslint-plugin/blob/v1.0.2/docs/rules/block-scoped-var.md',
    },
    fixable: undefined,
    schema: [],
    type: 'suggestion',
  ...
  create(context) {
    /**
         ...
         * @param {import('eslint').Rule.Node} node - A node to get variables.
         * @returns {void}
         */
